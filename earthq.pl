...
# This uses the Near Real Time Earthquake List and xplanet to show 
# earthquakes on your desktop
#
...
#
# Usage: * call the script.  It will write a markerfile 'quake' to xplanet/marker/
#        * put '-markerfile quake' in your xplanet commandline
#
# Copyright 2002 Hans Ecke <hans@ecke.ws>
#           after ideas from Michael Dear <Michael.Dear@collegiate.co.uk>
#
...
#
# ChangeLog: Version
#        2.1.7: filter out more errors in source data
#        2.1.6: EARTHQ_NEW_MINSIZE
#        2.1.5: stability fixes
#        2.1.4:
#        2.1.3: windows fixes
#        2.1.2:
#        2.1.1: fixed/changed EARTHQ_OLD_MINSIZE meaning and added EARTHQ_OLD variable
#          2.1: auto-updating; improved config file handling   
#        2.0.1: improved earthquake pictures, thanks Markus Hennig
#          2.0: divide source up into perl modules that only at distribution time get recombined
#               new unified configuration system using the xplanet.conf file
#               changed license to GPL
#               update webpage parsing
#          1.0: bumped release number to 1.0. this is not beta software anymore 
#               added more stringent checks in the website parsing
#       0.9.12: update caching
#       0.9.11: caching get_webpage()
#               windows and non-US compatibility fixes, thanks Stefan P. Wolf
#               fixed month and year in writedebug() thanks Stefan P. Wolf
#       0.9.10: more template examples, changed default
#               thanks Chris Harper for suggestion
#        0.9.9: -d command line option
#        0.9.8: updated parsing and locations because USGS changed everything
#               thanks Chris Harper for alerting me that the old version did not 
#                   work anymore
#        0.9.7: one more line of debug output
#               time format configurable. default works under Windows.
#               thanks Matz Johansson for the error report
#        0.9.6: added big earthquakes from the last 30 days thanks to
#               http://neic.usgs.gov/neis/qed/qed.html
#        0.9.5: more time fixes. hopefully its working correctly now....
#        0.9.4: added 'info' in the template syntax
#        0.9.3: added 'shorttime' in the template syntax   
#               got rid of the 'align=below' in the default templates
#        0.9.2: support for the last earthquake
#               time fixes
#        0.9.1: better template syntax
#          0.9: initial release
#
# Todo: info on last earthquake in side box?
...
sub mag2color($) {
   ...
   return 'paleGreen'        if $mag < 3.5;
   return 'Green'            if $mag < 4.0;
   return 'GreenYellow'      if $mag < 4.5;
   return 'ForestGreen'      if $mag < 5.0;
   return 'OliveDrab'        if $mag < 5.5;
   return 'khaki'            if $mag < 6.0;
   return 'LightGoldenrod'   if $mag < 6.5;
   return 'Yellow'           if $mag < 7.0;
   return 'salmon'           if $mag < 7.5;
   return 'orange'           if $mag < 8.0;
   return 'coral'            if $mag < 8.5;
   return 'tomato'           if $mag < 9.0;
   return 'OrangeRed'        if $mag < 9.5;
   return 'Red'              if $mag < 10;
   return 'aquamarine';
}

# the file we try to download
my $nrtel_location = 'http://neic.usgs.gov/neis/bulletin/bulletin.html';
my $thirtyday_location='http://neic.usgs.gov/neis/qed/qed.html';
   ...
   'check'  => {
      ...
      'EARTHQ_TEMPLATE' => {
         'comment'  => [
            ...
            '   $intmag     - the integer (whole number) part of the magnitude, i.e. "4" if the magnitude is 4.6',
            '   $color      - an associated color, dependant on magnitude',
            '   $size       - an associated fontsize, dependant on magnitude',
            '   $detail     - where it happened, as a description',
            '   $dep        - depth in kilo-meters',
            '   $time       - a string describing the time when it happened (in terms of your local time)',
            '   $shorttime  - a shorter string describing the time when it happened (in terms of your local time)',
            '   $info       - a smarter information string:',
            '                 "$mag"               for normal earthquakes',
            '                 "$mag [$shorttime]"  for the very latest earthquake',
            '                 "$mag [$shorttime]"  for earthquakes with magnitude above 5.5',
            '   <CR>        - line break, to make 2 or more markerlines that correspond to one earthquake ',
            '',
            'You can have templates that go over multiple lines, if you so desire',
            '',
            'Example: Use provided images ',
            '         $lat $long "$info" color=$color image=mag$intmag.png transparent={255,255,255} # $detail $time',
            '',
            'Example: Write a "O" where the EQ is, with fontsize dependant on magnitude.',
            '         Multiple lines per EQ.',
            '         $lat $long "O" image=none align=center color=yellow fontsize=$size # $info<CR>$lat $long "$info" image=none color=yellow transparent={255,255,255} # $detail $time',
            '',
            'Example: Just use different sized circles',
            '         $lat $long symbolsize=$size color=yellow fontsize=$size # $info<CR>$lat $long "$info" image=none color=yellow transparent={255,255,255} # $detail $time',
            ],
      ...
      'EARTHQ_OLD' => {
         'comment'  => [            
            'At which age is an earthquake considered "old"? The below number (in days)',
            'says that.',
            '',
            'Example: No earthquake is considered old',
            '         1000',
            '',
            'Example: Every earthquake is considered old',
            '         0',
            '',
            'Example: Earthquakes are considered old after 1 day',
            '         1',
            ],
         'default'  => 1,
         'nr'       => $conf_offset + 3,
      ...
      'EARTHQ_NEW_MINSIZE' => {
         'comment'  => [            
            'New earthquakes are shown if their magnitude is above this number.',
            'Set it to 0 to display all new earthquakes.',
            'Set it to some other magnitude to display only earthquakes above that magnitude.',
            ],
         'default'  => 0,
         'nr'       => $conf_offset + 4,
         },                 
      'EARTHQ_OLD_MINSIZE' => {
         'comment'  => [            
            'Older earthquakes are shown if their magnitude is above this number.',
            'Set it to very high (i.e. 100 or so) to only display very recent earthquakes.',
            'Set it very low to display all earthquakes of the last 30 days.',
            ],
         'default'  => 6.2,
         'nr'       => $conf_offset + 5,
         },                 
...
my $quake_marker_file     = File::Spec->catfile($xplanet_markers_dir,"quake");

my $template              = $cfg{'EARTHQ_TEMPLATE'};
my $template_last         = $cfg{'EARTHQ_TEMPLATE_LAST'};
my $template_old          = $cfg{'EARTHQ_TEMPLATE_OLD'};
($template,$template_last,$template_old)=map {s/\<CR\>/\n/g;$_} ($template,$template_last,$template_old);

my $old_earthquakes_min_size= $cfg{'EARTHQ_OLD_MINSIZE'};
is_numeric($old_earthquakes_min_size) || die "EARTHQ_OLD_MINSIZE $old_earthquakes_min_size is not numeric\n";

my $new_earthquakes_min_size= $cfg{'EARTHQ_NEW_MINSIZE'};
is_numeric($new_earthquakes_min_size) || die "EARTHQ_NEW_MINSIZE $new_earthquakes_min_size is not numeric\n";

my $old_earthquakes_age   = $cfg{'EARTHQ_OLD'};
is_numeric($old_earthquakes_age) || die "EARTHQ_OLD $old_earthquakes_age is not numeric\n";
$old_earthquakes_age *= 60*60*24;
...
sub help {
  ...
  $Script: download and create a marker file of recent earthquakes to be
  used with xplanet.
  ...
  \"quake\" marker file inside xplanet's marker directory.
...
# in: information hash describing one earthquake and its representation
...
sub expand_template(%) {
   ...
   elsif($eq{'age'}>$old_earthquakes_age) {
      return if $eq{'mag'}<$old_earthquakes_min_size;
      $text=$template_old;
      }
   else {   
      return if $eq{'mag'}<$new_earthquakes_min_size;
      $text=$template;
      }
   
   my ($lat,$long,$detail,$depth,$mag,$time) = @eq{'lat','long','detail','dep','mag','time'};
   
   my $size  = mag2size($mag);
   my $color = mag2color($mag);
   ...
   my $detail1 = sprintf("%-15s",$detail);
   my $detail2 = sprintf("%-17s",'"'.$detail.'"');

   my $shorttime=strftime($time_format, localtime($time));
   $shorttime =~ s/\s+/ /g;
   $time = localtime($time);

   $color = sprintf("%-20s",$color);

   my $mag1=sprintf("%s",$mag);
   my $mag2=sprintf("%-5s",'"'.$mag.'"');
   my $intmag=sprintf("%s",int($mag));

   my $info="$mag";
   if(($eq{'lastquake'})||($mag>=$old_earthquakes_min_size)) {
      $info="$mag [$shorttime]";
      }
   my $info1=sprintf("%s",$info);
   my $info2=sprintf("%-25s",'"'.$info.'"');
   
   $text =~ s/\$lat/$lat_str/g;
   $text =~ s/\$long/$long_str/g;
   $text =~ s/"\$detail"/$detail2/g;
   $text =~ s/\$detail/$detail1/g;
   $text =~ s/\$dep/$depth/g;
   $text =~ s/"\$mag"/$mag2/g;
   $text =~ s/\$mag/$mag1/g;
   $text =~ s/"\$info"/$info2/g;
   $text =~ s/\$info/$info1/g;
   $text =~ s/\$time/$time/g;
   $text =~ s/\$shorttime/$shorttime/g;
   $text =~ s/\$color/$color/g;
   $text =~ s/\$size/$size/g;
   $text =~ s/\$intmag/$intmag/g;
...
#
# get and pre-parse the quake file
#

my $quaketxt;

# get the quakes of the last 30 days: use only the biggest
$quaketxt=get_webpage($thirtyday_location);
if($quaketxt) {
   RECORD: foreach(split(/<tr>/i,$quaketxt)) {
      chomp;
      my @a=split("</td>",$_);
      pop @a;
      next RECORD if !@a;   
      foreach(@a) {
         ...
         s/&nbsp;/ /g;
         s/\s+/ /g;
         next RECORD if !/^<td headers="\w+"(?: align="\w+")?>\s*<font face="helvetica">\s*(.*?)\s*<\/font>$/;
         $_=$1;
         }
#<td headers="t1"> <font face="helvetica"><a href="qed_iqai.html">2002/09/03 07:08:52.2</a></font>
#<td headers="t2" align="right"><font face="helvetica"> 33.919</font>
#<td headers="t3" align="right"><font face="helvetica"> -117.764</font>
#<td headers="t4" align="right"><font face="helvetica"> 7</font>
#<td headers="t5" align="right"><font face="helvetica">4.6 </font>
#<td headers="t7"> <font face="helvetica"><a href="qed_iqai.html">GREATER LOS ANGELES AREA, CALIFORNIA.</a></font>
      if(scalar(@a) != 6) {   
         warn "did not understand record ".join("|",@a)." (".scalar(@a)." elems instead of 6)\n";
         next RECORD;
         }   
      my ($datetime,$lat,$long,$dep,$mag,$detail)=@a;
      if($datetime !~ /<a href=".*?">(.*?)<\/a>/) {
         warn "did not understand datetime string $datetime\n";
         next RECORD;
         }
      $datetime = $1;
      if($detail !~ /<a href="(.*?)">(.*?)<\/a>/) {
         warn "did not understand detail string $detail\n";
         next RECORD;
         }
      $detail =~ s/\.$//;
      my $url;
...
# get the quakes of the last 2 days: use all of them
$quaketxt=get_webpage($nrtel_location);
if($quaketxt) {
   RECORD: foreach(split(/<tr>/i,$quaketxt)) {
      chomp;
      my @a=split("</td>",$_);
      ...
      foreach(@a) {
         ...
         s/&nbsp;/ /g;
         s/\s+/ /g;
         next RECORD if !/^<td headers="\w+" align="\w+" valign="top">\s*<font face="helvetica">\s*(.*?)\s*<\/font>$/;
         $_=$1;
         }
   #<td headers="t1" align="center" valign="top"><font face="helvetica"><a href="neic_iqai.html"onMouseOver="window.status=\'&#060;PAS> SOUTHERN CALIFORNIA 02/09/03 07:08:52\'">2002/09/03 07:08:52</a></font>
   #<td headers="t2" align="right" valign="top"><font face="helvetica">33.92N</font>
   #<td headers="t3" align="right" valign="top"><font face="helvetica">117.76W</font>
   #<td headers="t4" align="right" valign="top"><font face="helvetica">7.3</font>
   #<td headers="t5" align="center" valign="top"><font face="helvetica">4.6</font>
   #<td headers="t6" align="center" valign="top"><font face="helvetica"> </font>
   #<td headers="t7" align="left" valign="top"><font face="helvetica"><a href="neic_iqai.html" onMouseOver="window.status=\'&#060;PAS> SOUTHERN CALIFORNIA 02/09/03 07:08:52\'">&#060;PAS> SOUTHERN CALIFORNIA</a></font>
      my($datetime,$lat,$long,$dep,$mag,$detail,$url);
      if(scalar(@a) == 6) {   
         ($datetime,$lat,$long,$dep,$mag,$detail)=@a;
         }
      else {
         warn "did not understand record ".join("|",@a)." (".scalar(@a)." elems instead of 6)\n";
         }   
      #<a href="neic_lchi.html"onMouseOver="window.status='CENTRAL ALASKA 02/11/04 01:09:27'">2002/11/04 01:09:27</a>
      if($datetime !~ /<a href=".*?"\s*onMouseOver="window.status=.?'.*?'">(.*?)<\/a>/) {
         warn "did not understand datetime string $datetime\n";
         next RECORD;
         }
      $datetime = $1;
      if($detail !~ /<a href="(.*?)"\s*onMouseOver="window.status=.?'.*?'">(.*?)<\/a>/) {
         warn "did not understand detail string $detail\n";
         next RECORD;
         }
      $detail =~ s/\.$//;
...
die "could download no earthquake data\n" if !@data;

#   
# now process the raw info to our internal representation
#

foreach my $quake (@data) {

   # split datetime into date and time
   my ($date,$time)=split(" ",$quake->{'datetime'},2);
   $quake->{'date'}=$date;
   $quake->{'time'}=$time;

   # parse the date/time strings (they are UTC) into UNIX seconds and store in 'time'
   if($date !~ /(\d\d)\/(\d\d)\/(\d\d)/) {
      $quake->{'time'}=0;
      next;
      }
   my ($year,$mon,$day) = ($1,$2,$3);
   $mon--;   # it expects month to start at 0=January
   if($time !~ /(\d\d):(\d\d):(\d\d)/) {
      $quake->{'time'}=0;
      next;
      }
   ...
   $quake->{'age'}=$start_time-$t;
   
   # parse lat/long into our signed notation
   my $lat = $quake->{'lat'};
   my $long= $quake->{'long'};
   my $sign;
   if($lat =~ /(\d+\.\d+)([NS])/) {
   ...
   # magnitude: just take off the M and make it a number
   $quake->{'mag'}=0 if !$quake->{'mag'};
   $quake->{'mag'} =~ s/M$//;
   is_numeric($quake->{'mag'}) || die "magnitude $quake->{'mag'} is not numeric\n";
   
   # detail: nicer capitalisation
   $quake->{'detail'} = lc($quake->{'detail'});
   $quake->{'detail'} =~ s/\b(\w)/uc($1)/eg;
   
   # make sure depth is a number
   is_numeric($quake->{'dep'}) || die "depth $quake->{'dep'} is not numeric\n"
   }


# no duplicates
# we can't go by latitude/longitude because those are not exact values:
# sometimes you have 24.430 and 24.429 denoting the same event
#
# also, if the 'old' list and the 'new' list have same members, 
# the 'old' list will be overwritten because it came first
{
my %data_t;
foreach my $quake (@data) {
   $data_t{$quake->{'mag'},$quake->{'time'}}=$quake;
   }
@data=sort {$b->{'time'} <=> $a->{'time'}} grep {$_->{'time'}} values %data_t;   
}
...
# output   
local *QUAK;
open(QUAK,">$quake_marker_file") || die("Could not open quake markers $quake_marker_file for writing: $!\n");;
print QUAK "# Quake marker file created by $Script version $VERSION\n";
print QUAK "# For more information read the top of the $Script script or go to\n";
print QUAK "# $homepage\n";
print QUAK "\n";
foreach my $quake (@data) {
   my $txt=expand_template(%$quake);
   next if !$txt;
   writedebug($txt);
   print QUAK "$txt\n";
   }
