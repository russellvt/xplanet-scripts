...
# This script collects the weather forecast for any location
# from www.weather.com and writes a markerfile that displays this 
# forecast on your xplanet maps
#
# INSTALL: Put this script in your xplanet directory
#          Run it once
#          It should create or update the xplanet.conf configuration file
#          Adjust FORECAST_XXX variables inside that file. You should only
#          have to change FORECAST_WEATHERCOM_CODE
#
# If xplanet complains about "marker file syntax error", you are running a version of
# xplanet that is faulty. You can find a fixed version at
#          http://hans.ecke.ws/xplanet#forecastpl
#
# homepage/newest version: http://hans.ecke.ws/xplanet
#
# Usage: * call the script.  It will write a markerfile 'forecast' to xplanet/marker/
#        * put '-markerfile forecast' in your xplanet commandline
#
...
#
# ChangeLog: Version 
#             1.2.2   you can disable icons with below's $have_icons variable
#             1.2.1   windows fixes
#               1.2   a title string for the forecast
#                     changed position configuration: FORECAST_POSITION instead of 
#                       FORECAST_XSIDE and FORECAST_YSTART
#             1.1.2   use the generic xplanet-table interface for drawing
#                     how many days forecast is configurable
#                     source code cleanup
#             1.1.1   weather.com changed its page layout, so the parsing had to be updated
#                     thanks Olaf Mueller-Michaels for the error report
#               1.1   auto-updating; improved config file handling
#               1.0   source code cleanup
#                     added support for Celsius temperatures, thanks Lars Hoyer
#                     new unified configuration system using the xplanet.conf file
#                     changed license to GPL
#               0.9   source cleanup, share source code between all xplanet perl modules
#             0.7.2   added non-US locations, thanks Gerrit Cap
#                     added choice of left/right display
#                     renamed from zipforecast.pl to forecast.pl, thanks Gerrit Cap 
#             0.7.1   updated caching
#             0.7     initial effort
#
...
# whether to show icons in the forecast table
my $have_icons=1;

# weather.com icons size (width=height)
my $icon_size=31;
...
my %cfg=check_param(
   'file'   => $conffile,
   'remove' => ['FORECAST_XSIDE','FORECAST_XSTART','FORECAST_YSTART'],
   'check'  => {
      $ONEPARAMFILE_DIR_OPTS    => \%ONEPARAMFILE_DIR_OPTS,
      $ONEPARAMFILE_METRIC_OPTS => \%ONEPARAMFILE_METRIC_OPTS,
      $ONEPARAMFILE_US_OPTS     => \%ONEPARAMFILE_US_OPTS,
      'FORECAST_WEATHERCOM_CODE' => {
         'comment'  => [
            'IF you are in the US:        ',
            '   * put your ZIP code in here',
            '',
            'OR if you are not in the US: (example: Berlin, Germany)',
            '   * go to www.weather.com with your browser',
            '   * type the location (Berlin) in the "local forecast" search box',
            '   * choose the correct city from the list of options',
            '   * when you have the forecast, scroll down to the "10 days forecast"',
            '   * click on "Printable Forecast"',
            '   * look at the URL of the current page: take the last element (GMXX0007) ',
            '     and put it into the location variable below',
            ],
         'default'  => '80401',
         'nr'       => 10,
         },
      'FORECAST_POSITION' => {
         'comment'  => [
            'The position of the forecasts-table, in the form',
            'XxY',
            '',
            'X: How far from the left/right corner we should draw the forecast',
            '   Negative numbers denote distance from the right',
            'Y: How far from the top we should start to draw the forecast',
      ...
      'FORECAST_DAYS' => {
         'comment'  => [
            'Set this to the number of forecast you\'d like. 10 is the maximum.',
            'This is approximately equal to the number of days of forecast you will',
            'be getting.',
            ],
         'default'  => 5,
         'nr'       => $conf_offset + 2,
...
# where the forecast marker will be written to
my $forecast_marker_file  = File::Spec->catfile($xplanet_markers_dir,"forecast");

my $US                    = $cfg{$ONEPARAMFILE_US_OPTS};

my $LOCATION              = $cfg{'FORECAST_WEATHERCOM_CODE'};

my $FAHRENHEIT            = !$cfg{$ONEPARAMFILE_METRIC_OPTS};

my $position              = $cfg{'FORECAST_POSITION'};
$position =~ /^(\-?\d+)x(\-?\d+)$/ || die "FORECAST_POSITION $position is not of the form XxY\n";
my ($x_start,$y_start)=($1,$2);

my $forecast_days         = $cfg{'FORECAST_DAYS'};
(is_numeric($forecast_days) and ($forecast_days>0) and ($forecast_days<11)) 
   or die "forecast days $forecast_days is not a number between 1 and 10\n";

my $URL = $US ? 
          "http://www.weather.com/weather/print/$LOCATION" : 
          "http://www.w3.weather.com/outlook/travel/print/$LOCATION";

#          "http://www.w3.weather.com/outlook/travel/print/http://www.weather.com/outlook/travel/print/$LOCATION"
writedebug("URL: $URL");          
...
sub help {
  ...
  $Script: Writes a weather forecast for your US ZIP code into a
  xplanet markerfile, using information from www.weather.com.
  ...
  \"forecast\" marker file inside xplanet's marker directory.
...
sub print_data($@) {
   ...
   # add table header to table start
   unshift @data, {
      'date'     => "Date",
      'iconfile' => "",
      'fore'     => 'Forecast',
   ...
   foreach my $dat (@data) {
      my @row;
      my ($date,$file,$fore,$temp,$precip)=@$dat{'date','iconfile','fore','temp','precip'};
      $file = abs2rel($file,$xplanet_images_dir) if $file;
      push @row,{'txt'   => $date,
                 'color' => 'white',
                };
      push @row,{'icon'  => $file,
...
my $title;  # contains forecast title
my @data;  # contains info hashes

{ # get and parse the forecast file
my $txt=get_webpage($URL,'check_redirect'=>1);
die "could not download $URL\n" if !$txt;
$txt =~ s/&nbsp;/ /g;
$txt =~ s/\s+/ /g;

$txt =~ /<TD CLASS="titlebar" VALIGN="TOP" ALIGN="LEFT">Forecast for (.*?)<\/TD>/
   or die "could not find title in $URL. Please contact author at $author_email\n";
$title=$1;   

#<B>High /<BR> Low (&deg;F)</B>
my $ori_temp_unit='F';
$txt=~/\<B\>High \/\<BR\> Low \(\&deg\;(\w)\)\<\/B\>/ and $ori_temp_unit=$1;
$ori_temp_unit eq 'F' 
   or $ori_temp_unit eq 'C' 
   or die "could not understand temperature unit $ori_temp_unit. Please contact the author at $author_email\n";
foreach(split("<TR>",$txt)) {
   my @a=split(/<TD CLASS="?inDent\w+"?>/,$_);
   next if scalar(@a)!=6;
   my @a2= @a;
   foreach(@a2) {
      s/\<\!\-\- .*? \-\-\>//g;
   ...
   my (undef,$date,$icon,$fore,$temp,$precip)=@a2;
   
   $icon =~ /<IMG SRC="?(http:\/\/image\.weather\.com.*?)"? WIDTH="?\d+"? HEIGHT="?\d+"? BORDER="?0"?(?: ALT="")?>/
      || die "did not understand IMG tag $icon";
   $icon=$1;
   my $icon_file=File::Spec->catfile($xplanet_images_dir,"forecast_".URL_2_filename($icon));
   cache_weather_icon($icon,$icon_file) if $have_icons;
   
   $temp =~ s/&deg;//g;
   $temp =~ s/N\/A/NA/g;
   my($low,$high)=map {$_=undef if $_ eq "NA";$_} split("/",$temp);
   
   is_numeric($low)  || die "low $low is not numeric in temperature $temp\n"   if defined $low;
   is_numeric($high) || die "high $high is not numeric in temperature $temp\n" if defined $high;
   
   # normalize temperatures to fahrenheit
   if($ori_temp_unit eq 'C') {
      $low =convert_units($low,'C','F') if defined $low;
      $high=convert_units($low,'C','F') if defined $high;
      }
   # and convert to celsius as needed   
   if(!$FAHRENHEIT) {   
      $low =convert_units($low,'F','C')  if defined $low;
      $high=convert_units($high,'F','C') if defined $high;
      }
   my $scale_name=($FAHRENHEIT ? 'F' : 'C');   
   if(defined $low) {
      $temp=sprintf("%.0f",$low);
      }
   else {
...
die "Could not gather any forecast information. Either www.weather.com is temporarily down or this script ($Script) is faulty.\n" 
   if !@data;
...
# output   
local *MA;
open(MA,">$forecast_marker_file") || die("Could not open forecast markers $forecast_marker_file for writing: $!\n");;
print MA "# forecast marker file created by $Script version $VERSION\n";
print MA "# For more information read the top of the $Script script or go to\n";
print MA "# $homepage\n";
print MA "\n";
print MA print_data($title,@data);
close MA;   
