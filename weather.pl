...
# ############################################################################
#
# Copyright 2001 João Pedro Gonçalves joaop@co.sapo.pt for version 1
# Copyright 2001, 2002 Hans Ecke hans@ecke.ws for all versions>=2
#
...
my %cfg=check_param(
   ...
   'check'  => {
      ...
      'WEATHER_YAHOO_URL' => {
         'comment'  => [
            'Where to get the Yahoo weather information.',
            'Please go to this page and check that it really display your weather information',
            '',
            'Example: If you don\'t want yahoo weather',
            '         <leave it empty>',
            '',
            'Example: From weather.yahoo.com',
      ...
      'WEATHER_WUNDERGROUND_URL' => {
         'comment'  => [
            'Where to get the Weather Underground weather information.',
            'Please go to this page and check that it really display your weather information',
            '',
            'Example: If you don\'t want weather underground weather',
            '         <leave it empty>',
            '',
            'Example: Standard URL',
            '         http://www.wunderground.com/cgi-bin/findweather/getForecast?addfav=',
            ],
         'default'  => 'http://www.wunderground.com/cgi-bin/findweather/getForecast?addfav=',
         'nr'       => $conf_offset + 1,
         },
      'WEATHER_TEMPLATE' => {
         'comment'  => [
            'How each marker looks like.',
            'Known variables:',
            '   $lat       - latitude',
            '   $long      - longitude',
            '   $temp      - temperature',
            '   $icon      - weather icon',
            '   $city      - city name',
            '   $condition - condition string, something like "Cloudy"',
            '   <CR>   - add a linebreak. This enables you to have several marker lines for one weather record',
            '',
            'Example: Show icon, temperature and city name (note this is two lines!)',
            '         $lat $long "$temp" align=below color=orange image=$icon transparent={255,0,255}   # $city<CR>$lat $long "$city" color=red',
            '',
            'Example: Show icon and temperature',
            '         $lat $long "$temp" align=below color=orange image=$icon transparent={255,0,255}   # $city',
            ],
         'default'  => '$lat $long "$temp" align=below color=orange image=$icon transparent={255,0,255}   # $city',
         'nr'       => $conf_offset + 2,
         },
      'WEATHER_NOICON_TEMPLATE' => {
         'comment'  => [
            'A similar template, if we can not find an icon for the weather condition.',
            'Understands above variables, except:',
            '   _NOT_       $icon',
            '',
            'Example: Show condition and temperature',
            '         $lat $long "$condition [$temp]" image=none color=orange transparent={255,0,255}   # $city',
            ],
         'default'  => '$lat $long "$condition [$temp]" image=none color=orange transparent={255,0,255}   # $city',
         'nr'       => $conf_offset + 3,
         },
      },
);
...
# the weather marker file we write out
my $weather_m_fn          = File::Spec->catfile($xplanet_markers_dir,"weather");

my $YAHOO_URL             = $cfg{'WEATHER_YAHOO_URL'};
my $WUNDERGROUND_URL      = $cfg{'WEATHER_WUNDERGROUND_URL'};
$YAHOO_URL || $WUNDERGROUND_URL || die "Configuration error: Please set either the variable YAHOO_URL or WUNDERGROUND_URL or both.\n";

my $template              = $cfg{'WEATHER_TEMPLATE'};

my $template_noicon       = $cfg{'WEATHER_NOICON_TEMPLATE'};

my $locationslog=File::Spec->catfile($xplanet_dir,"log.weathericon-locations");

get_webpage(undef);
die "$Scriptbase needs cookies enabled. Please set COOKIES_FILE in $conffile\n" if !$HAVE_COOKIES;

sub help() {
   print <<EOM;

  $Script: download weather information from weather.yahoo.com or weather
  underground (wunderground.com) for usage with xplanet.
  ...
  \"weather\" marker file inside xplanet's marker directory.
  ...
  Installation:
     ...
     3.) Read the Configuration section on top of the script and follow
         the instructions.
...
# hash  uc(city name) => [ latitude longitude] if information is available
#       uc(city name) => [] if no info available
my %coords;

# in: list of city names
#
# out: nothing
#
# fills %coords with info about each city, if it can find it
sub get_all_coords(@) {
   my (@cities)=@_;

   @cities=grep {!exists $coords{uc($_)}} @cities;
   return if !@cities;

   Geo::SingleLocator::single_init();
   foreach my $city (@cities) {
      my %info=$LOCATOR->get_exact($city);
      my ($lat,$long)=@info{'lat','long'};
      next if !$lat;
      $coords{uc($city)}=[$lat,$long];
      }
...
# in: URL, filename of weather icon
# action: if not existing, grap it from the URL and put into
# $xplanet_images_dir
sub cache_weather_icon($$) {
   my ($url,$file)=@_;
   if(!get_webpage($url, 'file' => $file,'cache_expiration' => -1)) {
...
# in: latitude, longitud, temperature, icon filename, city
...
sub expand_template(%) {
   ...
   if($info{'file'}) {
      ...
      $text=$template_noicon;
      }

   my ($lat,$long,$temp,$city,$icon,$condition)=@info{'lat','long','temp','city','file','condition'};
...
#   my $temp1 = sprintf("%-10s",$temp);
   my $temp1 = $temp;
   my $temp2 = sprintf("%-12s",'"'.$temp.'"');
   my $city1 = sprintf("%-15s",$city);
   my $city2 = sprintf("%-17s",'"'.$city.'"');

   $icon="weather_".$icon if defined $icon and $icon ne '';

   $text =~ s/\$lat/$lat_str/g;
   $text =~ s/\$long/$long_str/g;
   $text =~ s/"\$temp"/$temp2/g;
   $text =~ s/\$temp/$temp1/g;
   $text =~ s/\$icon/$icon/g if $icon;
   $text =~ s/\$condition/$condition/g;
   $text =~ s/"\$city"/$city2/g;
   $text =~ s/\$city/$city1/g;
...
# get and process the weather.yahoo.com webpage
# return array hashes that contain temperature, image file, and location name information
sub get_weather_yahoo($) {

   my ($url)=@_;

   my @coords_to_get;   # return values

   #
   # get the webpage
   #

   my $page = get_webpage($url);
   if(!$page) {
      warn "could not download $url\n";
      return;
      }
   ...
   for(split(/<tr>/,$page)) {
      ...
      my @a=split(/<td.*?>/);
      @a=grep {$_} map {s/<\/td>//g;s/<\/tr>//g;s/^\s+//;s/\s+$//;$_} @a;
      ...
      foreach ($city,$temp,$gif,$condition) {
         s/^\s+//;
         s/\s+$//;
         }
      ...
      local *LOG;
      open(LOG,">> $locationslog") || die "could not open $locationslog\n";
      print LOG "$gif\n";
      close(LOG);
      $file = URL_2_filename($gif);
      if(!cache_weather_icon($gif,File::Spec->catfile($xplanet_images_dir,"weather_$file"))) {
         warn "Bailing from location $city\n";
         $city="";
         next;
         }

      push @coords_to_get,{ 'temp'       => $temp,
                            'file'       => $file,
                            'city'       => $city,
                            'condition'  => $condition,
                            };
   ...
   return @coords_to_get;
}


sub do_nothing() {}

# get and process the weather underground webpage
# return array hashes that contain temperature, image file, and location name information
sub get_weather_wunderground($) {

   my ($url)=@_;

   # try to use Yahoo's gifs for weather underground's textual conditions
   my %cond2gif=(
      'Blowing Snow'                 => '15.gif',
      'Blowing Widespread Dust'      => '21.gif',
      'Clear'                        => '32.gif',
      'Drizzle'                      => '9.gif',
      'Freezing Fog'                 => '8.gif',
      'Fog'                          => '22.gif',
      'Haze'                         => '20.gif',
      'Heavy Rain'                   => '12.gif',
      'Heavy Thunderstorms and Rain' => '38.gif',
      'Ice Pellets'                  => '6.gif',
      'Light Drizzle'                => '9.gif',
      'Light Freezing Drizzle'       => '8.gif',
      'Light Haze'                   => '20.gif',
      'Light Low Drifting Snow'      => '14.gif',
      'Light Mist'                   => '34.gif',
      'Light Rain Showers'           => '11.gif',
      'Light Rain'                   => '11.gif',
      'Light Snow Showers'           => '13.gif',
      'Light Snow'                   => '13.gif',
      'Light Thunderstorms and Rain' => '38.gif',
      'Low Drifting Snow'            => '15.gif',
      'Mist'                         => '34.gif',
      'Mostly Cloudy'                => '28.gif',
      'N/A'                          => '999.gif',
      'Overcast'                     => '26.gif',
      'Partly Cloudy'                => '30.gif',
      'Patches of Fog'               => '21.gif',
      'Rain'                         => '12.gif',
      'Rain Showers'                 => '11.gif',
      'Sand'                         => '21.gif',
      'Scattered Clouds'             => '30.gif',
      'Shallow Fog'                  => '22.gif',
      'Smoke'                        => '20.gif',
      'Snow Showers Freezing Fog'    => '8.gif',
      'Snow'                         => '14.gif',
      'Thunderstorm'                 => '4.gif',
      'Unknown'                      => '999.gif',
      'Widespread Dust'              => '21.gif',
      );

   my @coords_to_get;   # return value
...
RECORD:   for(split(/<tr/,$page)) {
      ...
      s/^\s+//;
      s/\s+$//;
      next RECORD if ! /^BGCOLOR="#[0-9a-fA-F]{6}"\s*>(.*)/i;
      my $part=$1;
      $part =~ s/\s*<\/tr>\s*$//;
      my @a=map {s/\s*<\/td>\s*$//;s/^\s+//;s/\s+$//;$_;} split(/<td.*?>/, $part);
#         bgcolor=#FFFFFF >          <td >  <a href="/global/stations/89009.html">Amundsen-Scott, Antarctica</a></td>   <td>      -48&#176; C             </td>   <td>N/A          </td>   <td> -             </td>   <td>Ice Pellets     </td>   <td>6:00 PM NZST</td>   <td>&nbsp;</td> <td> <a href=/php/editfavs.php?delfav=00000.89009><img src="http://icons.wunderground.com/graphics/DEL.gif" WIDTH=30 HEIGHT=16 BORDER=0></a> <a href=/php/editfavs.php?up=00000.89009><img src="http://icons.wunderground.com/graphics/UP.gif" WIDTH=16 HEIGHT=16 BORDER=0></a><a href=/php/editfavs.php?down=00000.89009><img src="http://icons.wunderground.com/graphics/DOWN.gif" WIDTH=16 HEIGHT=16 BORDER=0></a> </td></tr>
#         bgcolor=#EEEEEE>           <td>   <a href="/US/CO/Denver.html">         Denver, Colorado          </a></td>   <td>      26&#176; C              </td>   <td>25%          </td>   <td> 1022&nbsp;hPa </td>   <td>Scattered Clouds</td>   <td>9:53 AM MDT </td>   <td> &nbsp; </td> <td> <a href=/php/editfavs.php?delfav=80002.3><img src="http://icons.wunderground.com/graphics/DEL.gif" WIDTH=30 HEIGHT=16 BORDER=0></a> <a href=/php/editfavs.php?up=80002.3><img src="http://icons.wunderground.com/graphics/UP.gif" WIDTH=16 HEIGHT=16 BORDER=0></a><a href=/php/editfavs.php?down=80002.3><img src="http://icons.wunderground.com/graphics/DOWN.gif" WIDTH=16 HEIGHT=16 BORDER=0></a> </td></tr> </table> </center> <br> <!-- blue credits bar --> <! -- serial # 041801-1 --> <table width=100%>

      do_nothing();

      next RECORD if scalar(@a) != 9;

      my ($city,$temp,$hum,$press,$cond,$updated,$wind);
      (undef,$city,$temp,$hum,$press,$cond,$wind,$updated,undef)=@a;
      ...
      ($temp=~/([-\d]+)\s*&#176;\s*([CF])/) && ($temp="$1\' $2");

       my $file=$cond2gif{$cond};

       if((!defined $file) or ($file eq '')) {
          warn "Could not find icon for weather condition $cond at location $city. You might want to contact the author at $author_email and send him the file $locationslog\n";
          }
       elsif($file eq 'none') {
          writedebug("We don't have an icon for condition $cond for location $city. Using textual representation.");
          $file=undef;
          }
       else {
          writedebug("Translating condition \"$cond\" to icon $file");
          if(!cache_weather_icon("$default_weather_icon_location/$file",
                             File::Spec->catfile($xplanet_images_dir,"weather_$file"))) {
             warn "Bailing from location $city\n";
             $city="";
             next;
             }
...
# the processors for each url
# this is overkill for only 2 sources, but allows for further extension
my %url_processor;
$url_processor{$YAHOO_URL}       =\&get_weather_yahoo        if $YAHOO_URL;
$url_processor{$WUNDERGROUND_URL}=\&get_weather_wunderground if $WUNDERGROUND_URL;

# gather weather information

my %results;       # url -> array of result hashes
my %coord_to_get;  # hash of all city names to locate
foreach my $url (keys %url_processor) {
   my @this_result = $url_processor{$url}->($url);
   next if !@this_result;
   $results{$url}=\@this_result;
   my @cities_found;
   foreach (@this_result) {
      my $city=$_->{'city'};
      if($coord_to_get{$city}) {
         warn "City $city encountered more than once. Is this correct?\n";
         }
      else {
         $coord_to_get{$_->{'city'}}=1;
         }
      push @cities_found,$city;
      }
   writedebug("Found locations ".join(", ",@cities_found));
   }

# call out to geo_locator for locations
get_all_coords(keys %coord_to_get);

# add the coords to the information hashes
# delete any elements where we can't find any coords

foreach my $url (keys %results) {
   my @this_result=@{$results{$url}};
   my @new_result;
   foreach (@this_result) {
      my $city       = $_->{'city'};
      if((!$coords{uc($city)}) || (!@{$coords{uc($city)}}))  {
         writedebug("Could not find info to city $city");
         next;
         }
      my ($lat,$long)= @{$coords{uc($city)}};
      $_->{'lat'} =$lat;
      $_->{'long'}=$long;
      push @new_result,$_;
      }
   if(@new_result) {
      $results{$url}=\@new_result;
      }
   else {
      delete $results{$url};
      }
...
foreach my $url (keys %results) {
   ...
   my @this_lines;
   foreach (@this_result) {
      my $text=expand_template(%$_);
      writedebug($text);
      push @this_lines, $text;
      }
   if(@this_lines) {
      push @lines, "# $url";
      push @lines, "";
      push @lines, @this_lines;
      push @lines, "";
      }
   }
pop @lines;
...
if (@lines) {
   local *MARK;  # the weather marker file we write
   open(MARK,">$weather_m_fn") || die("Could not open weather markers $weather_m_fn for writing: $!\n");;
   print MARK "# Weather marker file created by $Script version $VERSION\n";
   print MARK "# For more information read the top of the $Script script or go to\n";
   print MARK "# $homepage\n";
   print MARK "\n";
   foreach(@lines) {
      print MARK $_."\n";
   ...
   die "Could not gather any weather information\n"
   }
