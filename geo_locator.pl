...
# of a given location using exact or inexact searches. It can also
# perform reverse lookups.
#
# The first call to geo_locator.pl might take a little time so it can initialize its cache
#
# See the associated README.geo_locator file for more information
# or just call "geo_locator.pl -h"
...
# in which format to write the output, 'tzcoord' or 'tzcoord_exact' or 'marker'
# needs to be up here for output()
my $opt_outmode='tzcoord';  
...
my $opt_mode='s';        # r=reverse d=dump s=search

my $inmode='l';          # l=location f=file
...
my %cfg=check_param(
   ...
   'check'  => {
      ...
      'GEOLOCATOR_MARKERS' => {
         'comment'  => [
            'The list of marker files where geo_locator.pl gets its location information from,',
            ...
            'the scripts execution be. Especially the census2000_* files are big.',
            '',
         ...
         'default'  => 'weather_markers earth capitals nations ed_u.com bcca.org',
         'nr'       => $conf_offset,
         },
      },                
...
my @earth_m_fn            = markerlist_2_array($xplanet_markers_dir,$cfg{'GEOLOCATOR_MARKERS'});

########################################################################
#
#
#   Global variables. Only allowed in this section.
#
#
########################################################################
...
#
#
#   Program logic - Utility functions
#
#
########################################################################

# output an information hash
sub output(%) {
   my (%hit)=@_;
   ...
   # tzcoord.pl - like output
   if($opt_outmode eq 'tzcoord') {
      $p = "-lat ".print_coord($hit{'lat'}).
          " -lon ".print_coord($hit{'long'}).
          " -city ".dquote(print_name($hit{'match'}));
      $p.=" -detail ".dquote($hit{'detail'}) if $hit{'detail'};
      $p.=" -dist ".print_angle($hit{'dist'}) if $hit{'dist'};
      }
   # tzcoord.pl's output mode is strictly only latitude and longitude
   elsif($opt_outmode eq 'tzcoord_exact') {
      $p = "-lat ".print_coord($hit{'lat'}).
          " -lon ".print_coord($hit{'long'});
      }
   # xplanet-style marker file
   elsif($opt_outmode eq 'marker') {
      $p =print_coord($hit{'lat'})." ".print_coord($hit{'long'})." ".sprintf("%-40s",dquote(print_name($hit{'match'})));
      $p.=" # $hit{'detail'}" if $hit{'detail'}; 
      $p.=", dist:".print_angle($hit{'dist'})."\"" if exists $hit{'dist'}; 
      }
   else {
      die "unknown output mode $opt_outmode found\n";
      }   
...
#
#
#   Processing each mode
#
#   These functions do no searching by themselves, only I/O
#
#
########################################################################
...
# out: success/failure
#
# action: looks up the search, outputs any results if there are any
sub lookup() {

   my @hits=$LOCATOR->lookup();
   return 0 if !@hits;
...
# print all location information to STDOUI
#
# out: 1 (success)
sub process_dump_locations() {

   foreach my $city ($LOCATOR->dump_locations()) {
      output(%$city);
      }
...
# in: file of locations to read and look up; search mode
#
# out: number of hits found
sub process_masslookup($) {
   my ($infile) = @_;
   
   if($opt_outmode eq 'marker') {
      print "# Generated by $Script version $VERSION\n";
      print "# locations from input file $infile\n";
      print "# For more information see $homepage\n";
      print "\n";
      }
   ...
   writedebug("accepting search strings from $infile");
   ...
   writedebug("Hit $count_hits out of $count_all.");
...
sub help() {
   my $help=<<EOM;
Usage:
  $Script [-n] [-l <location type>] [-s 1|t|m] <location> <location> <location>...
  $Script [-n] [-l <location type>] [-s 1|t|m] -f <infile>
  $Script [-n] [-l <location type>] [-s 1|t|m] < infile
  $Script -r <latitude>,<longitude>
  $Script --dump
  $Script -h
Options for any mode:
  --angle <minimal angle>
  --outmode (tzcoord|marker|tzcoord_exact)
  --zonetab <zone.tab file>
  
  Tries to determine the coordinates (latitude and longitude) of
  a given location using xplanet (http://xplanet.sourceforge.net)
  marker files or the timezone file installed on your system. Can
  also search Thesaurus of Geographical Names at the Getty Institute
  ...
  -e, --detail    Filter for the specified string inside the description 
                  i.e. \"$Script -e USA leipzig\" will find a location called 
                  \"leipzig" in the US, if the description contains that string.               
  --dump          Dump all local location information
  -f <infile>     Read search strings from <infile>
  ...
  -o, --outmode   Either \"tzcoord\" (default), \"marker\" or \"tzcoord_exact\".
                  In which format to print the information.
                  \"marker\": xplanet's marker file format
                  \"tzcoord\": the format tzcoord.pl uses to print coordinates
                  \"tzcoord_exact\": exactly like tzcoord.pl
  -r, --reverse   Reverse lookup - finds closest match to given coordinates
                  Coordinates are given as a pair of latitude and longitude,
                  i.e. 39.74,-104.98
...
EOM
   ...
   if(@earth_m_fn) {
      $help.="  Configured and found markerfiles:\n".
             "      ".join("\n      ",@earth_m_fn);
      }
   else {   
      $help.="  Currently no markerfiles are available for lookup. Please edit the\n".
             "  configuration section in xplanet.conf.";
      }
...
my ($qlat, $qlong);      # coords requested for reverse lookup

my $infile;              # file to read locations from

my $opt_angle=undef;         # the angle we give on the command line

my $opt_reverse='';      # the latitude,longitude pair - gets parsed in $qlat and $qlong

my $opt_reverse_angle=0; # if 0 - return nearest neighbor to point else return all inside the circle

my $opt_loctype='';      # location type requested from TGN

my $opt_detail='';       # detail filter requested from TGN
...
die "Problems with the command line. Aborting\n" if ! GetOptions(
       ...
       'net|n:s'     =>   \@net_modules,
       'outmode|o=s' =>   \$opt_outmode,
       'reverse|r=s' =>   \$opt_reverse,
...
$opt_angle=distspec2deg($opt_angle);
...
if($opt_reverse) {
   $opt_mode='r';
   ($qlat,$qlong)=split(/,/,$opt_reverse,2);
   die "Invalid latitude $qlat given\n"   if not_numeric($qlat);
   die "Invalid longitude $qlong given\n" if not_numeric($qlong);
   $opt_reverse_angle=$opt_angle if defined $opt_angle;
   }
if(($opt_outmode ne "tzcoord")&&($opt_outmode ne "marker")&&($opt_outmode ne "tzcoord_exact")) {
   die "Unknown output mode $opt_outmode requested\n";
   }
...
# last resort: use zone.tab file taht came with geo_locator.pl
...
if($opt_mode eq 'f') {
   writedebug("getting locations from infile $infile");
   }
elsif($opt_mode eq 'r') {
   writedebug("reverse lookup: [$qlat, $qlong] dist=$opt_reverse_angle");
   }   
elsif($opt_mode eq 'd') {
   writedebug("dumping all known locations...");
   }   
if($opt_loctype) {   
   writedebug("location type: $opt_loctype");
   }
if($opt_net) {   
...
writedebug("output mode: $opt_outmode");
...
$LOCATOR=Geo::Locator->new(
                            ...
                            'qlat'          => $qlat,
                            'qlong'         => $qlong,
                            'reverse_angle' => $opt_reverse_angle,
                            'mode'          => $opt_mode,
...
if($infile){
   $inmode='f';
   die "Could not find location search file $infile\n" if ! -f $infile;
   die "Could not read location search file $infile\n" if ! -r $infile;
   }
if(($opt_mode eq 's') && (!@ARGV)){   
   if($I_am_interactive) {
      writestderr("You did not give me a location to search for. I will now accept location strings from STDIN (typed into the terminal), till you quit this session by typing $EOF_CHAR_WRITTEN");
      }
   $inmode='f';
   $infile="-";
   }   
...
my $success=1;
if($opt_mode eq "d" ) {
   $success=process_dump_locations();
   }
elsif($opt_mode eq "r" ) {
   $success=lookup();
   }
elsif($infile) {
   $success=process_masslookup($infile);
   }
else{
   $success=0;
   foreach my $city (@ARGV) {
      $LOCATOR->city($city);
      my $this_succ=lookup();
      $success ||= $this_succ;   # if at least one is returned, return success
      }
   }   

# return value of program has to be reversed from success
my $ret=0;
$ret=1 if !$success;
exit $ret;
