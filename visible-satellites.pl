...
# This uses www.heavens-above.com and xplanet to show positions and movements of
# visible satellites on your desktop
#
# INSTALL: Put this script in your xplanet directory
#          Call "visible-satellites.pl -t" and pick your timezone from the list
#          Edit xplanet.conf: Set VISIBLE_SATELLITES_TIMEZONE to the timezone you picked
#                             Adjust any other variables you like
#
# homepage/newest version: http://hans.ecke.ws/xplanet
#
# Usage: * call the script.  It will write a satellite marker files 'visible-satellites' to xplanet/satellites
#        * put '-satfile visible-satellites' in your xplanet commandline
#
...
#            0.7: windows fixes
#            0.6: polishing..
#            0.5: initial release after an idea by Markus Hennig
...
my %cfg=check_param(
   ...
   'check'  => {
      $ONEPARAMFILE_DIR_OPTS      => \%ONEPARAMFILE_DIR_OPTS,
      $ONEPARAMFILE_US_OPTS       => \%ONEPARAMFILE_US_OPTS,
      'VISIBLE_SATELLITES_MAGNITUDE' => {
         'comment'  => [
            'The minimal magnitude of brightness. Possible:',
            '   3.5 - Only the brightest satellites',
            '   4.0 - Medium',
            '   4.5 - Even the dimmest satellites',
            ],
         'default'  => "3.5",
         'nr'       => $conf_offset,
         },
      'VISIBLE_SATELLITES_TIMEZONE' => {
         'comment'  => [
            'Which timezone you are in, in terms heavens-above.com can understand.',
            "Please call \"$Script -t\", pick a value from the left column (i.e. Egypt or UCTm4)",
            'and place it here.',
            ],
         'default'  => "",
         'nr'       => $conf_offset+1,
         },
      'VISIBLE_SATELLITES_TEMPLATE' => {
         'comment'  => [
            'How visible satellites are shown. Understands:',
            '   $satid     - Satellite ID, necessary for xplanet to know what satellite we are',
            '                talking about',
            '   $start2max - Number of minutes from start of visibility to maximum visibility',
            '   $max2end   - Number of minutes from maximum visibility to the end of visibility',
            '   $usmaxtime - A string representing the time of maximum visibility, with AM/PM',
            '                like custom in the US. I.e. 7:30pm instead of 19:30.',
            '                If we have multiple sightings, the different times are comma',
            '                separated',
            '   $eumaxtime - A string representing the time of maximum visibility, with 24 hour',
            '                clock like custom in the EU. I.e. 19:30 instead of 7:30pm.',
            '                If we have multiple sightings, the different times are comma',
            '                separated',
            '   $maxtime   - Either $usmaxtime or $eumaxtime, depending on whether you live in the',
            '                US or not (you set this with the "US" key at the top of this file)',
            '   $name      - Satellite name',
            '   $mag       - Magnitude of brightness. Remember, a smaller number means brighter!',
            '                If we have multiple sightings, the different magnitudes are comma',
            '                separated',
            '   <CR>       - Linebreak, for having multiple lines per satellite',
            ],
         'default'  => '# $name [$mag]<CR>$satid "$name ($maxtime)" color=white<CR>$satid "" image=none altcirc=45 trail={orbit,-$start2max,0,1} color=white<CR>$satid "" image=none trail={orbit,0,$max2end,1} color=red',
         'nr'       => $conf_offset+2,
         },
      },                
...
my $satfile               = File::Spec->catfile($xplanet_satellites_dir,"visible-satellites");
my $tlefile               = File::Spec->catfile($xplanet_satellites_dir,"visible-satellites.tle");

my $mag=$cfg{'VISIBLE_SATELLITES_MAGNITUDE'};
is_numeric($mag) || die "brightness magnitude $mag is not a number\n";
$mag==3.5 or $mag==4.0 or $mag==4.5 or die "brightness magnitude $mag not correct value (one of 3.5, 4.0, 4.5)\n";

my ($lat,$long)           = determine_location();
die "could not determine your home location\n" if ! (defined $lat and defined $long);

my $template              = $cfg{'VISIBLE_SATELLITES_TEMPLATE'};
$template =~ s/\<CR\>/\n/g;

my $US                    = $ONEPARAMFILE_US_OPTS;
...
sub help {
  ...
  $Script: download and create a satelite marker file of
  satellites that will be visible today from your home location.
  
  This script understands only 1 command line options:
  
     -d(ebug): turn on debug mode, in which the program will output 
               additional information. This is usefull for tracking down 
               errors.
  ...
  \"visible-satellites\" satellites file inside xplanet's satellites 
  directory.
...
# got this:
# from website source, take into file t.t, then execute
# cat t.t|perl -n -e'/OPTION VALUE="(\w+)"(?: SELECTED)?>(.*)/ || die "could not understand $_";my ($v,$w)=($1,$2);print "   [".sprintf("%-15s","\"$v\"").", \"$w\"],\n"' > t.t.t
my @TZs=(
   ["UCT11"        , "(GMT-11) American Samoa, Midway Is., Niue Is., Samoa"],
   ["AlST"         , "(GMT-10) USA (Aleutian Is.)"],
   ["UCT10"        , "(GMT-10) USA (Hawaii), Tahiti, Cook Is., Johnston Is."],
   ["NAST"         , "(GMT-9) USA (Alaska)"],
   ["PST"          , "(GMT-8) USA (Pacific), Canada (Pacific & Yukon), Mexico (Baja N.)"],
   ["Arizona"      , "(GMT-7) USA (Arizona)"],
   ["MST"          , "(GMT-7) USA (Mountain), Canada (Mountain)"],
   ["UCT7"         , "(GMT-7) Mexico (Baja S.)"],
   ["CST"          , "(GMT-6) USA (Central), Canada (Central)"],
   ["Sask"         , "(GMT-6) Canada (Saskatchewan)"],
   ["UCT6"         , "(GMT-6) Belize, Costa Rica, El Salvador, Guatemala, Honduras, Mexico, Nicaragua"],
   ["ACRE"         , "(GMT-5) Brazil (Trinity of Acre)"],
   ["EST"          , "(GMT-5) USA (Eastern), Canada (Eastern), Bahamas, Haiti, Turks & Cacos"],
   ["Indiana"      , "(GMT-5) USA (Indiana)"],
   ["UCT5"         , "(GMT-5) Cayman Is., Colombia, Ecuador, Jamaica, Panama"],
   ["AST"          , "(GMT-4) Canada (Atlantic), Bermuda, Greenland (Thule)"],
   ["Chile"        , "(GMT-4) Chile"],
   ["UCT4"         , "(GMT-4) Barbados, Bolivia, Dominican Republic, Guadeloupe, Guiana, Puerto Rico, Venezuela"],
   ["WBST"         , "(GMT-4) Brazil (West)"],
   ["NST"          , "(GMT-3.5) Canada (Newfoundland)"],
   ["EBST"         , "(GMT-3) Brazil (East, including all coast and Brasilia)"],
   ["NAST3"        , "(GMT-3) Saint Pierre & Miquelon"],
   ["SAT"          , "(GMT-3) Argentina, French Guiana, Surinam, Uruguay"],
   ["NORO"         , "(GMT-2) Brazil (Fernando De Noronha)"],
   ["EUT1"         , "(GMT-1) Greenland (Scoresbysund), Portugal (Azores)"],
   ["UCT1"         , "(GMT-1) Cape Verde Is"],
   ["GMT"          , "(GMT+0) United Kingdom, Ireland"],
   ["PWT"          , "(GMT+0) Portugal, Madeira"],
   ["UCT"          , "(GMT+0) Iceland, Mali, Ivory Coast, Gambia, Ghana, Guinea, Liberia, Morocco, Senegal"],
   ["WET"          , "(GMT+0) Spain (Canary Is.), Faroe Islands"],
   ["CET"          , "(GMT+1) Austria, Belgium, Denmark, France, Germany, Italy, Netherlands, Norway, Spain, Sweden, Switzerland"],
   ["UCTm1"        , "(GMT+1) Algeria, Angola, Cameroon, Chad, Congo, Gabon, Niger, Nigeria, Tunisia"],
   ["EET"          , "(GMT+2) Bulgaria, Finland, Greece, Romania, Russian Fed. (Zone1), Turkey, Ukraine"],
   ["Egypt"        , "(GMT+2) Egypt"],
   ["EUTm2"        , "(GMT+2) Lebanon"],
   ["Israel"       , "(GMT+2) Israel"],
   ["JST"          , "(GMT+2) Jordan"],
   ["RFTm2"        , "(GMT+2) Russian Federation (Zone 1)"],
   ["SAST"         , "(GMT+2) South Africa"],
   ["SST"          , "(GMT+2) Syria"],
   ["UCTm2"        , "(GMT+2) Botswana, Burundi, Estonia, Mozambique, Rwanda, Sudan, Swaziland, Zambia, Zimbabwe"],
   ["IST"          , "(GMT+3) Iraq"],
   ["RFTm3"        , "(GMT+3) Russian Federation (Zone 2)"],
   ["UCTm3"        , "(GMT+3) Ethiopia, Kenya, Kuwait, Madagascar, Qatar, Saudi Arabia, Somalia, Tanzania"],
   ["Iran"         , "(GMT+3.5) Iran"],
   ["RFTm4"        , "(GMT+4) Russian Federation (Zone 3)"],
   ["UAEST"        , "(GMT+4) United Arab Emirates"],
   ["UCTm4"        , "(GMT+4) Azerbaijan, Mauritius, Oman, Reunion, Seychelles, United Arab Emirates"],
   ["AFG"          , "(GMT+4.5) Afghanistan"],
   ["RFTm5"        , "(GMT+5) Russian Federation (Zone 4)"],
   ["UCTm5"        , "(GMT+5) Maldives, Pakistan, Tajikistan, Turkmenistan, Uzbekistan"],
   ["UCTm5colon30" , "(GMT+5.5) India"],
   ["UCTm5colon45" , "(GMT+5.75) Nepal"],
   ["RFTm6"        , "(GMT+6) Russian Federation (Zone 5)"],
   ["UCTm6"        , "(GMT+6) Bangladesh, Bhutan, Sri Lanka"],
   ["UCTm6colon30" , "(GMT+6.5) Myanmar, Cocos (Keeling) Is."],
   ["RFTm7"        , "(GMT+7) Russian Federation (Zone 6)"],
   ["UCTm7"        , "(GMT+7) Cambodia, Kampuchea, Indonesia (West), Thailand, Vietnam"],
   ["ChST"         , "(GMT+8) China, Hong Kong, Taiwan"],
   ["MALST"        , "(GMT+8) Malaysia, Singapore, Indonesia (Central), Philippines"],
   ["RFTm8"        , "(GMT+8) Russian Federation (Zone 7)"],
   ["UCTm8"        , "(GMT+8) Australia (Western), Brunei"],
   ["JapST"        , "(GMT+9) Japan"],
   ["RFTm9"        , "(GMT+9) Russian Federation (Zone 8)"],
   ["UCTm9"        , "(GMT+9) Indonesia (East), Korea"],
   ["ACST"         , "(GMT+9.5) Australia (South Australia and Broken Hill)"],
   ["UCTm9colon30" , "(GMT+9.5) Australia (Northern Territory)"],
   ["AEST"         , "(GMT+10) Australia (NSW, Capitol Territory, Victoria)"],
   ["RFTm10"       , "(GMT+10) Russian Federation (Zone 9)"],
   ["TST"          , "(GMT+10) Australia (Tasmania)"],
   ["UCTm10"       , "(GMT+10) Australia (Queensland), Guam, Papua New Guinea"],
   ["RFTm11"       , "(GMT+11) Russian Federation (Zone 10)"],
   ["UCTm11"       , "(GMT+11) New Caledonia, Solomon Is., Vanuatu"],
   ["UCTm11colon30", "(GMT+11.5) Norfolk Is."],
   ["NZST"         , "(GMT+12) New Zealand"],
   ["RFTm12"       , "(GMT+12) Russian Federation (Zone 11)"],
   ["UCTm12"       , "(GMT+12) Fiji, Kiribati, Nauru, Tuvalu, Wake Is., Wallis & Futana Is."],
   ["UCTm13"       , "(GMT+13) Tonga"],
   );
...
foreach (@TZs) {
   ...
   if($key =~ /^(\(GMT[+-][\d\.]+\))\s*(.*)$/) {
      my ($gmt,$other)=($1,$2);
      $key=sprintf("%-11s",$gmt).$other;
      }
   $TZs.="   ".sprintf("%-15s",$val)." $key\n";
   $TZs{$val}=$key;
   }

sub help_tz() {
   my $msg="";
   $msg.="Here is the list of timezones heavens-above.com recognizes.\n";
   $msg.="\n";
   $msg.="Please choose the correct timezone from the list. Place the identifier in the left\n";
   $msg.="column (like RFTm10 or UCTm12) into VISIBLE_SATELLITES_TIMEZONE in $conffile\n";
...
my $TZ                    = $cfg{'VISIBLE_SATELLITES_TIMEZONE'} || 
     die "You have not yet set a timezone. Please call \"$Script -t\", pick a timezone and set the VISIBLE_SATELLITES_TIMEZONE value in xplanet.conf\n";
$TZs{$TZ} || die "timezone $TZ not recogniced. Please choose again\n";   
...
sub expand_template(%) {
   ...
   my ($name,$orbit,$satid)=@sat{'name','orbit','satid'};
   my @encs=@{$sat{'enc'}};
   
   my (@start,@max,@end,@start_to_max,@max_to_end,@eumaxtime,@usmaxtime,@mag);
   foreach(@encs) {
      my %enc=%$_;
      my ($start,$max,$end,$mag)=@enc{'start_time','max_time','end_time','mag'};
      push @start_to_max,($max-$start)/60;
      push @max_to_end,($end-$max)/60;
      push @mag,$mag;
      my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime($max);
      $mon++;
      $year+=1900;    
      my $ushour=$hour;
      my $ampm="am";
      ...
      foreach($mon,$mday,$hour,$ushour,$min,$sec) {   
         $_ ="0$_"  if $_<10;   
         }
      push @eumaxtime,"$hour:$min";
      push @usmaxtime,"$ushour:$min$ampm";
      }
   my $eumaxtime=join(", ",@eumaxtime);
   my $usmaxtime=join(", ",@usmaxtime);
   my $maxtime=($US ? $usmaxtime : $eumaxtime);
   my $mag=join(", ",@mag);  
   my $start_to_max=int($start_to_max[0]);  # take the first encounter's time
   my $max_to_end  =int($max_to_end[0]);    # take the first encounter's time
   
   $text =~ s/\$satid/$satid/g;
   $text =~ s/\$start2max/$start_to_max/g;
   $text =~ s/\$max2end/$max_to_end/g;
   $text =~ s/\$maxtime/$maxtime/g;
   $text =~ s/\$usmaxtime/$usmaxtime/g;
   $text =~ s/\$eumaxtime/$eumaxtime/g;
   $text =~ s/\$name/$name/g;
   $text =~ s/\$mag/$mag/g;
...
sub tstr_2_sec($) {
   ...
   $str =~ /^(\d+):(\d+):(\d+)$/ || die "could not understand time string $str\n";
   ($hour,$min,$sec)=($1,$2,$3);
   return timelocal($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst);
}   
...
#         'mag'
#         'enc' => list of hashes
#             'start_time'  seconds local time
#             'start_alt'
#             'start_az'
#             'max_time'    seconds local time
#             'max_alt'
#             'max_az'
#             'end_time'    seconds local time
#             'end_alt'
#             'end_az'
sub understand_main_satlist() {
   my $URL="http://www.heavens-above.com/allsats.asp?lat=$lat&lng=$long&alt=0&loc=Unspecified&TZ=$TZ&Mag=$mag";
   my $html=get_webpage($URL) || die "could not retreive $URL.\n";
   $html =~ s/\s+/ /g;
   my %data;
   my @recs=split(/<TR>/,$html);
   foreach my $rec (@recs) {
      $rec =~ s/<\/TR>.*//;
      my @d=split(/<TD>/,$rec);
      next if scalar(@d)<5;
      @d=map {s/<\/TD>.*//;$_} @d;
      @d=grep {$_} map {s/^\s+//;s/\s+$//;$_} @d;
      if(scalar(@d)!=11) {
         warn "did not understand record ".join("<TD>",@d)."\n";
         next;
         }
      my ($nameurl,$mag,$start_time,$start_alt,$start_az,$max_time,$max_alt,$max_az,$end_time,$end_alt,$end_az)=@d;
      $nameurl =~ /SatID=(\d+)/ || die "could not find satellite-id in URL $nameurl\n";
      my $satid=$1;
      $nameurl    = strip_anchor($nameurl);
      $start_time = tstr_2_sec(strip_anchor($start_time));
      $max_time   = tstr_2_sec(strip_anchor($max_time));
      $end_time   = tstr_2_sec(strip_anchor($end_time));
      $start_alt =~ s/&#176;$//;
      $max_alt   =~ s/&#176;$//;
      $end_alt   =~ s/&#176;$//;
      is_numeric($mag) || die "magnitude $mag of $nameurl is not numeric?\n";
...
my %sats=understand_main_satlist();

sub get_orbit(%) {
   my (%sat)=@_;
   my $name=$sat{'name'};
   my $satid=$sat{'satid'};
   my $URL="http://www.heavens-above.com/orbitdisplay.asp?lat=$lat&lng=$long&alt=0&loc=Unspecified&TZ=$TZ&satid=$satid";
   my $html=get_webpage($URL) || die "could not retreive $URL.\n";
   $html =~ /<PRE>(.*?)<\/PRE>/s || die "orbit for $name not found ($URL)";
   $html=$1;
   $html =~ s/<\/?\w+>//g;
   $html =~ s/^\s+//;
   $html =~ s/\s+$//;
   $html =~ s/\r//g;
   $html || die "orbit for $name is empty ($URL)\n";
   $html =~ /^([\d\s\w\.\-\+]+)\n([\d\s\w\.\-\+]+)$/ || die "orbit for $name: $html not understood ($URL)\n";
   $html ="$1\n$2";
   return $html;
}   

foreach my $sat (values %sats) {
   $sat->{'orbit'}=get_orbit(%$sat);
   }
   
local *SATFILE;
local *TLEFILE;

open(SATFILE,"> $satfile") || die "could not open $satfile for writing: $!\n";   
open(TLEFILE,"> $tlefile") || die "could not open $tlefile for writing: $!\n";   

print SATFILE "# Satellite marker file created by $Script version $VERSION\n";
print SATFILE "# For more information read the top of the $Script script or go to\n";
print SATFILE "# $homepage\n";
print SATFILE "\n";
...
sub print_info(%) {
   ...
   print TLEFILE "$name\n";
   print TLEFILE "$orbit\n";
   print SATFILE expand_template(%sat)."\n";
}
