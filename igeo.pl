...
# A simple interactive location search tool
#
# call "igeo.pl -h" for short help or start igeo.pl and type "h" for online help
#
# the first call to igeo.pl might take a little time so it can initialize its cache
#
# See the associated README.igeo file for more information
...
# the default zone.tab file (can be overridden on the command line
# if none can be found on your system we will use the zone.tab
# file that came with this package, so don't worry too much about this
my $zone_tab = '/usr/share/zoneinfo/zone.tab';

# command line arguments / global switches
my $min_angle_difference=2;
...
my %cfg=check_param(
   ...
   'check'  => {
      ...
      'IGEO_MARKERS' => {
         'comment'  => [
            'The list of marker files where igeo.pl gets its location information from,',
            'delimited by spaces. Do not include the directory portion, only the name.',
            '',
            'Available marker files are all that you can find in the markers/ subdirectory.',
            '',
            'The more and bigger marker files you specify here the slower will',
            'the scripts startup be. Especially the census2000_* files are big.',
            '',
            'Example: Just a few markers',
            '         weather_markers earth capitals nations ed_u.com bcca.org',
            '',
            'Example: All available markers',
            '         *',
            ],
         'default'  => '*',
         'nr'       => $conf_offset,
...
my $tmpdir                = $cfg{$ONEPARAMFILE_TMPDIR_OPTS} || $xplanet_dir;
-d $tmpdir                || die("Could not find directory $tmpdir\n") ;
-r $tmpdir                || die("Could not read directory $tmpdir\n") ;
-w $tmpdir                || die("Could not write directory $tmpdir\n") ;

my $local_cache           = File::Spec->catfile($tmpdir,"local_location.cache");

my @earth_m_fn            = markerlist_2_array($xplanet_markers_dir,$cfg{'IGEO_MARKERS'});

########################################################################
#
#
#   End of configuration section. Don't change anything below.
#
#
########################################################################
...
sub help_cmdln() { 
print <<EOM;
Usage : $Script <preset options>

Will start the interactive location finder. Type h for help
to see available options.

It can be started with some options preset (you can change everything once
inside the program):

-a, --angle <angle> If two locations are less than <minimal angle> (degrees)
                apart, consider them identical. If <angle> is 0, do not
                try to resolve duplicates.
                For reverse mode: if specified, return all matches inside a
                circle of <angle> radius.
                The angle can be specified as degrees, as US-miles (i.e. 
                \"2miles\" = 0.03 degrees) or kilo-meters (i.e. 
                \"2km\" = 0.02 degrees).
-d              Switch on loging. Usefull for error reports to the author.
-e, --detail    Filter for the specified string inside the description
                i.e. \"-e USA\" will only show matches in the US
-l, --loctype <type> Restrict online results to this location type
                eg. "inhabited place", nation, county, peak, river, bay
-n, --net       Search online database for matches if necessary. At the
                moment we only search the TGN (Getty's thesaurus)
-s, --search    Search mode:
                -s 1 : only deliver exact matches (default)
                -s t : deliver only exact (trusted) matches if possible
                -s m : deliver all matches, including substring matches
-z, --zonetab   zone.tab (timezone) file to use.
                Currently $zone_tab

-h              This help


All coordinates are given as signed floating point numbers. For example
the city Denver (in the US) has coordinates 39.74 -104.98. This means
39 degrees, 44.4 minutes North, 104 degrees, 58.5 minutes West.
...
my $opt_angle=undef;         # the angle we give on the command line

my $opt_loctype='';      # location type requested from TGN

my $opt_detail='';       # detail filter

my $opt_net=0;           # 0 = don't use TGN, 1 = use it

my $opt_search_mode='1'; # 1=get_coords() t=get_coords_multi_trust() m=get_coords_multi()

my @net_modules;         # list of internet datasources to use

die "Problems with the command line. Aborting\n" if ! GetOptions(
       'angle|a=f'   =>   \$opt_angle,
       'help|h'      =>   \&help_cmdln,
       'loctype|l=s' =>   \$opt_loctype,
       'detail|e=s'  =>   \$opt_detail,
       'net|n:s'     =>   \@net_modules,
       'search|s=s'  =>   \$opt_search_mode,
       'zonetab|z=s' =>   \$zone_tab,
       );
       
# process raw command line switches into program state

if(defined $opt_angle) {
   die "Please specify a positive angle, not $opt_angle\n" if $opt_angle<0;
   $min_angle_difference=$opt_angle;
   }
if(($opt_search_mode ne "1")&&($opt_search_mode ne "m")&&($opt_search_mode ne "t")) {   
   die "Unknown search mode $opt_search_mode requested\n";
   }
if(@net_modules) {    # is empty if no -net; is ('') if just -net; is filled if specified source
   $opt_net=1;   
   @net_modules=split(/,/,join(',',@net_modules));   
   @net_modules=grep {$_} map {s/^\s+//;s/\s+$//;s/[^\w_]//g;uc($_)} @net_modules;
   @net_modules=('TGN') if !@net_modules;
   }  
# compatibility with tzcoord.pl: if last argument is an absolute filename, assume thats zone.tab
$zone_tab=pop @ARGV if (@ARGV) 
                       && (-f $ARGV[$#ARGV]) 
                       && (-r $ARGV[$#ARGV]) 
                       && (File::Spec->file_name_is_absolute($ARGV[$#ARGV]));
$zone_tab=File::Spec->catfile($Bin,"zone.tab") if !-f $zone_tab;
die "Could not find timezone information file $zone_tab\n" if ! -f $zone_tab;
die "Could not read timezone information file $zone_tab\n" if ! -r $zone_tab;

# now output debug info about configuration:

writedebug("minimal angle: $min_angle_difference");
if($opt_net) {   
   writedebug("net modules: ".join(", ",@net_modules));
   }
else {
   writedebug("only local search requested");
   }   
writedebug("search mode: $opt_search_mode");
writedebug("zone.tab file $zone_tab requested");
...
sub output(%) {
   ...
   $p = "-lat ".print_coord($hit{'lat'}).
       " -lon ".print_coord($hit{'long'});
   $p.=" -city ".sprintf("%-25s",dquote(print_name($hit{'match'})))."" if $hit{'match'};
   $p.=" -dist ".print_angle($hit{'dist'}) if exists $hit{'dist'};
   $p.=" -detail ".dquote($hit{'detail'}) if $hit{'detail'};
...
$process_funcs{'h'}=sub{
   ...
   my %cmd2hlp=(
   'm'  => <<EOM ,
m <module name>              : add module
m !<module name>             : delete module
m                            : list loaded modules in order
m ?                          : list Perl packages detected that might 
                               be loadable modules
EOM
   'a'  => <<EOM , 
a <angle>                    : minimal angle difference [@{[print_angle($LOCATOR->angle_diff())]}]
                               two locations of a similar name that 
                               are only this far apart are considered
                               identical
a                            : print minimal angle difference
EOM
   's'  => <<EOM ,
s 1|t|m                      : search mode              [@{[$LOCATOR->smode()]}]
                               1 - exact search, preferred local 
                                   information
                               t - try exact search, if no results 
                                   do multi search
                               m - multi/substring search
s                            : print out current search mode
EOM
   'r'   => <<EOM ,
r <lat> <long> [<angle>]     : reverse search
                               if angle is specified, return all 
                               results inside that circle around 
                               (lat,long); otherwise return location 
                               nearest to (lat,long)
EOM
   'l'   => <<EOM ,
l <location type>            : filter for the specified location type 
                               [@{[$LOCATOR->loctype() || "none"]}]
                               a comma-separated list if you wish to 
                               apply an "or" filter
l none                       : delete location filter
l                            : which location type we are currently 
                               filtering for
EOM
   'e'   => <<EOM ,
e <detail>                   : filter for the specified string inside 
                               the "detail" information [@{[$LOCATOR->detail() || "none"]}]
                               This allows you i.e. to pick only 
                               locations in the US by specifying 
                               "d USA"
e none                       : delete detail filter
e                            : which detail string we are currently 
                               filtering for 
EOM
   'q'   => <<EOM , 
q                            : quit
EOM
   'h'   => <<EOM , 
h                            : long help
h <command>                  : help to a specific one-letter command.
                               Possible are [@{[join(", ",@OPTIONS)]}]
EOM
   );

   foreach(@OPTIONS) {
      die "did not find help for option $_\n" if !$cmd2hlp{$_};
      }
   foreach(keys %cmd2hlp) {
      die "help for unknown option $_ found\n" if !$OPTIONS{$_};
      }

   if($arg) {
      if($cmd2hlp{$arg}) {
         printout($cmd2hlp{$arg});
         return;
         }
      else {
         printout("Option $arg not understood. Possibilities are ".join(", ",@OPTIONS));
         return;
         }
      }      

   my $help_first="Welcome to $Script, an interactive location search tool";

   my $help_last=<<EOM;   
<location name>              : forward search

Loaded modules (in order):
@{[ do {

         my $ret="";
         my %modules=$LOCATOR->list_modules_descriptions();
         my @modules=$LOCATOR->list_modules();
         foreach my $mod (@modules) {
            my $desc=$modules{$mod};
            $ret.="      ".sprintf("%-10s",$mod)." \t $desc\n";
            }
         $ret;   
}]}
@{[ do {
   if(@earth_m_fn) {
      "Configured and found markerfiles:\n".
      "      ".join("\n      ",@earth_m_fn)."\n";
      }
   else {   
      "Currently no markerfiles are available for lookup. Please edit the\n".
      "configuration file xplanet.conf.\n";
      }
} ]}

zone.tab file: $zone_tab 
  
All coordinates are given as signed floating point numbers. For 
example the city Denver (in the US) has coordinates 39.74 -104.98. 
This means 39 degrees, 44.4 minutes North, 104 degrees, 58.5 minutes 
West.

Angle differences (the argument to the above \"a\" and \"r\" commands) 
can be specified as degrees, as US-miles (i.e. \"2miles\" = 0.03 
degrees) or kilo-meters (i.e. \"2km\" = 0.02 degrees).

Version: $VERSION
For more information see $homepage
Copyright 2002-2003 Hans Ecke and Felix Andrews under the terms 
of the GPL

For shorter help, type 'h h'
EOM
...
$process_funcs{'m'}=sub{
   ...
   if(!$mod) {
      ...
      foreach my $mod (@modules) {
         my $desc=$modules{$mod};
         printout(sprintf("%-10s",$mod)." \t $desc");
         }
      return;   
      }
      
   # return possible module list   
   if($mod eq '?') {
      my @list=possible_packages($LOCATOR->package_prefix());
      my %loaded= map {$_ => 1} $LOCATOR->list_modules();
      printout("Perl packages that might (or might not) be loadable locator modules");
      foreach my $pmod (@list) {
         if($loaded{$pmod}) {
            printout("      $pmod \t (loaded)");
            }
         else {
            printout("      $pmod");
            }   
   ...
   if($mod =~ /^\!\s*(.*)/) {
      my $mod=$1;
      my %modules= map {$_ => 1} $LOCATOR->list_modules();
      if($modules{$mod}) {
         $LOCATOR->delete_module($mod);
         printout(join("\n",$LOCATOR->list_modules()));
         }
      else {
         printout("Can not unload module $mod: it is not loaded");
         return;
         } 
      return;     
      }
   ...
   my %modules= map {$_ => 1} $LOCATOR->list_modules();
   if($modules{$mod}) {
      printout("Module $mod is already loaded");
      return;
      }
   my $ret=$LOCATOR->add_module($mod);
   if($ret) {
      printout(join("\n",$LOCATOR->list_modules()));
      }
   else {   
      printout("Could not add requested module $mod. It is probably not a locator plugin.");
      }
   };
...
$process_funcs{'a'}=sub{
   my ($ang)=@_;
   if($ang eq '') {
      printout("minimal angle difference is: ".$LOCATOR->angle_diff());
      return;
      }
   $ang=distspec2deg($ang);
   if(!defined $ang) {
      printout("$ang is not a distance. (h=help)");
      return;
      }
   if($ang>=0) {
      $LOCATOR->angle_diff($ang);
      printout("minimal angle difference is now: ".print_angle($LOCATOR->angle_diff()));
      }
   else {
      printout("Please give a positive angle difference, not ".print_angle($ang));
      }   
   };
...
$process_funcs{'s'}=sub{
   ...
   $arg=lc($arg);
   if(!$arg) {
      printout("search mode is: ".$LOCATOR->smode());
      }
   elsif($arg =~ /^[1tm]$/i) {
      $LOCATOR->smode($arg);
      printout("search mode is now: ".$LOCATOR->smode());
      }
   else {
      printout("Unknown search mode $arg encountered. h=help.");
      return;
      }
   };
...
$process_funcs{'l'}=sub{
   my ($arg)=@_;
   if(!$arg) {
      my $loctype=$LOCATOR->loctype() || "none";
      printout("location type filter set to: $loctype");
      }
   else {
      $arg=""     if $arg eq "none";
      $LOCATOR->loctype($arg);
      my $loctype=$LOCATOR->loctype() || "none";
      printout("location type filter is now set to: $loctype");
      }
   };
...
$process_funcs{'e'}=sub{
   my ($arg)=@_;
   if(!$arg) {
      my $detail=$LOCATOR->detail() || "none";
      printout("detail filter set to: $detail");
      }
   else {
      $arg=""     if $arg eq "none";
      $LOCATOR->detail($arg);
      my $detail=$LOCATOR->detail() || "none";
      printout("detail filter is now set to: $detail");
      }
  };
   
$process_funcs{'r'}=sub{
   my ($arg)=@_;
   my ($qlat,$qlong,$revang)=split(/\s+/,$arg,3);
   $qlat ='<none>' if !defined $qlat  or $qlat eq '';
   $qlong='<none>' if !defined $qlong or $qlong eq '';
   if(!is_numeric($qlat)) {
      printout("latitude $qlat is not a number. (h=help)");
      return;
      }
   $qlat=getnum($qlat);
   if(!is_numeric($qlong)) {
      printout("longitude $qlong is not a number. (h=help)");
      return;
      }
   $qlong=getnum($qlong);
   if(defined $revang and $revang ne '') {
      $revang=distspec2deg($revang);
      if(!defined $revang) {
         printout("angle $revang is not a number. (h=help)");
         return;
         }   
      if($revang<0) {
         printout("angle ".print_angle($revang)." is negative. Using ".print_angle(abs($revang))." instead");
         $revang=abs($revang);
         }   
      }
   my @ret=$LOCATOR->lookup_reverse($qlat,$qlong,$revang);
   if(@ret) {
      output_list(@ret);
      }
   else {   
      printout("No results. Type h for help, try other location or try loading more data sources");
      }
   };
   
foreach(@OPTIONS) {
   die "did not find processor for option $_\n" if !$process_funcs{$_};
   }
foreach(keys %process_funcs) {
   die "processor for unknown option $_ found\n" if !$OPTIONS{$_};
   }
...
sub process_opt($$) {
   ...
   $opt=lc($opt);
   if($process_funcs{$opt}) {
      return $process_funcs{$opt}->($arg);
      }
   else{   
      printout("Unknown option $opt. Type h for help.");
      return 0;
      }
}
...
sub process($) {
   ...
   if($line =~ /^([\w?])(?:\s+(.+))?$/) {
      ...
      my @ret=$LOCATOR->lookup_forward($line);
      if(@ret) {
         output_list(@ret);
         }
      else {   
         printout("No results. Type h for help, try other location, loading more data sources or changing the search mode");
         }
      }   
}


# we have to explicitely state we want to use STDIN and STDOUT so redirections etc work
my $term=Term::ReadLine->new('Interactive locator',*STDIN,*STDOUT);
$OUT = $term->OUT || *STDOUT;

if($I_am_interactive) {
   $prompt="igeo> ";
   printout("");
   printout("Welcome to igeo.pl, an interactive location search tool");
   printout("");
   printout("For long help type  \"h\"");
   printout("For short help type \"h h\"");
   printout("");
   }
else {  # make sure in non-interactive mode we do not output strange terminal ANSI sequences
   $prompt="> ";
   $term->ornaments(0);
   ($term->Features())->{'ornaments'}=0;
   }   
...
while ( defined ($line = $term->readline($prompt)) ) {
   ...
   foreach my $cmd (split(/;/,$line)) {
      ...
      $term->addhistory($cmd) if ($cmd =~ /\S/) && ((!$lastline) || ($lastline ne $cmd));
      
      $lastline=$cmd;
      
      }

   exit if (!$I_am_interactive) and eof($term->IN);
