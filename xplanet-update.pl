...
# the main clouds image we try to download
my $main_cloud_file="clouds_2000.jpg";

# where we download the SSEC clouds map from
my $ssec_cloud_url="http://www.ssec.wisc.edu/data/comp/wv/LATEST_WV.gif";
# what filename we give to the SSEC clouds map
my $ssec_cloud_file="ssec_clouds.gif";

my $seasurfacetemp_url="http://www.ssec.wisc.edu/data/sst/latest_sst.gif";
my $seasurfacetemp_file="sea_surface_temp.gif";
...
sub help() {
  ...
  To set $Script up,
     ...
     * Run it
     * Afterwards you will find an xplanet.conf file which you can
       further customize
...
my %cfg=check_param(
   ...
   'check'  => {
      ...
      'SCRIPTS' => {
         'comment'  => [
            'Which scripts do you wish to run?',
            '',
            '   * weather            : display the current weather maps from yahoo and weather underground',
            '   * earthq             : display recent earthquakes',
            '   * volcano            : display currently active volcanoes',
            '   * hurricano          : display tropic hurricane paths and positions',
            '   * forecast           : display a table of the weather forecast over the next 10 days for your area',
            '   * visible-satellites : show the positions and movements of satellites that are visible from your position',
            '   * moonphase          : display the moon at its current phase and position',
            '',
            'Add the ones you want to the SCRIPTS',
            ],
         'default'  => 'weather earthq volcano hurricane forecast visible-satellites moonphase',
         'nr'       => $conf_offset,
         },
      'CLOUDS_MIRRORS' => {
         'comment'  => [
      ...
      'DAY_IMAGE' => {
         'comment'  => [
            'The generic day image without clouds',
            ],
         'default'  => 'earth.jpg',
         'nr'       => $conf_offset + 2,
         },
      'NIGHT_IMAGE' => {
         'comment'  => [
            'The generic night image without clouds',
            ],
         'default'  => 'night.jpg',
         'nr'       => $conf_offset + 3,
         },
...
my @scripts               = grep {xplanet_script_2_file($_)} split(/\s+/,$cfg{'SCRIPTS'});
if(@scripts) {
   writedebug("found scripts ".join(", ",@scripts));
   }
else {
...
my $day_image             = $cfg{'DAY_IMAGE'};
-f File::Spec->catfile($xplanet_images_dir,"$day_image")
                          || die "could not find day image file $xplanet_images_dir/$day_image\n";
...
sub make_image_with_clouds($$$$) {
   my ($cloud_file,$naked_file,$xplanet_option,$out)=@_;
   if(!execute_xplanet(
             $xplanet_option." ".
             "-cloud_thresh $cloud_thresh ".
             "-blend ".
             "-geometry $geometry",
             'output' => File::Spec->catfile($xplanet_images_dir,$out),
             'image'  => File::Spec->catfile($xplanet_images_dir,$naked_file),
             )) {
      warn "could not generate a new $out image\n";
      unlink(File::Spec->catfile($xplanet_images_dir,$cloud_file));
      return 0;
      }
   return 1;
}      
...
sub make_main_clouds($) {
   ...
   return 0 if !make_image_with_clouds(
              $main_cloud_file,
              $night_image,
              "-cloud_shade $cloud_shade -cloud_image ".quote_fn($main_cloud_file),
...
#
# this can't be done portably:
#   which convert >/dev/null
#   if [ "$?" -eq "0" ]
#   then
#      convert -crop 1071x515+6+11 sea_surface_temp.gif -|convert -scale '2000x1000!' -  sea_surface_temp.new.jpg
#      if [ -s sea_surface_temp.new.jpg ]
#      then
#         mv sea_surface_temp.new.jpg sea_surface_temp.jpg
#         chmod 664 sea_surface_temp.jpg
#      fi
#   fi
#   rm sea_surface_temp.gif


# run all scripts
foreach my $script (@scripts) {
   execute_xplanet_script($script,"");
   }
